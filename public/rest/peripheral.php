...
function handle_get($pdo)
{
    ...
    if(isset($_GET["id"]))
    {
        $sql = "SELECT id, group_name FROM p_peripheral WHERE id = \"" . sanitize_string($_GET["id"]) . "\"";
    }
    else if(isset($_GET["group_name"]))
    {
        $sql = "SELECT id, group_name FROM p_peripheral WHERE group_name = \"" . sanitize_string($_GET["group_name"]) . "\"";
    }
    else
    {
    ...
    if(isset($_GET["group_name"]))
    {
        try
        {
            $sql = "INSERT INTO p_log (action, on_table, on_column, new_value, user)"
                . ' VALUES ("INSERT", "p_peripheral", ?, ?, ?)';
            $statement = $pdo->prepare($sql);

            $statement->execute(array("group_name", sanitize_string($_GET["group_name"]), $user_data["group_name"]));
            $data = $statement->fetchAll(PDO::FETCH_ASSOC);

            $sql = "INSERT INTO p_market_state (group_name) VALUES (" . sanitize_string($_GET["group_name"]) . ")";
            header('X-SQL: ' . $sql);
            $statement = $pdo->prepare($sql);
            $statement->execute();
            $data = $statement->fetchAll(PDO::FETCH_ASSOC);
...
{
    $user_data = authenticate($pdo);
    if((isset($_GET["id"])) && (isset($_GET["group_name"])))
    {
        $sql = "SELECT id, group_name FROM p_peripheral WHERE id = \"" . sanitize_string($_GET["id"]) . "\"";
    }
    if(isset($sql))
    {
        try
        {
            ...
            $sql = "INSERT INTO p_log (action, on_table, on_id, on_column, old_value, new_value, user)"
                . ' VALUES ("PUT", "p_peripheral", ' . $data['id']. ', ?, ?, ?, ?)';
            $statement = $pdo->prepare($sql);
            // name
            ...
            if($newName != $data['name'])
            {
                ...
                $data['name'] = $newName;
                // echo("now updating");
                $sql = "UPDATE p_peripheral SET group_name = ? WHERE id =?";
                $statement = $pdo->prepare($sql);
    ...
    if(isset($sql))
    {
        try
        {
            ...
            if(isset($data['id']))
            {
                ...
                . ' VALUES ("DELETE", "p_peripheral", ?, ?, ?, ?)';
                $statement = $pdo->prepare($sql);
                $statement->execute(array($data['id'], "group_name", $data['group_name'], $user_data["group_name"]));

                $sql = "DELETE FROM p_peripheral WHERE id =?";
                $statement = $pdo->prepare($sql);
